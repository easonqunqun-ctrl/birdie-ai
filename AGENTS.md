...
- **二期产品规划（v0.2.1 战略收敛 + 留存补强）** → [`docs/21-二期产品需求规划.md`](docs/21-二期产品需求规划.md)（M7 引擎 V2 / M8 教练工作台 双 P0；M12 职业对比 / M13 球友约球 P1-P2；球场 / 机构 / 社区 Feed 在三期）。
- **二期排期与 PLAN-ID 矩阵** → [`docs/22-二期开发迭代计划.md`](docs/22-二期开发迭代计划.md)（`P2-Mx-Nn` 命名；UI/交互延续一期，**不**引入新 UI 库）。配套真源：`docs/02` §十一 / `docs/03` §八 / `docs/05` §八 / `docs/06` §十三 / `docs/20` §6.2-6.3。
- **二期可编码规格 (FR / AC 清单)** → [`docs/23-二期可编码规格说明书.md`](docs/23-二期可编码规格说明书.md)（67 个 `P2-Mx-Nn` 全量展开；docs/01 文末已加反向链接，二期工作**不再回写 docs/01**；接 / 字段 / DDL 仅引用 docs/02 §十一 / docs/03 §八 锚点，不重复抄表；每 P2-Mx-Nn 含 用户场景 / FR / 接口契约 / 数据模型 / NFR / AC / 灰度回滚 / 文档同步 8 子节，FR ≥3、AC ≥2 且可量化；写代码前先按本表勾选）。
