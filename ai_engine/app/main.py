...
from app.errors import ModeClubMismatchError, PipelineError
...
def _mode_club_mismatch(req: AnalyzeRequest) -> ModeClubMismatchError | None:
    ...
    if req.mode == "full_swing" and category == "putter":
        return ModeClubMismatchError(
            f"mode=full_swing but club_type={req.club_type!r} is putter"
        )
    return None


def _mock_putting_result(req: AnalyzeRequest) -> AnalyzeResult:
    """mock 模式下的推杆报告（确定性，符合 schema）。"""
    from app.pipeline.putting.constants import PUTTING_PHASE_LABELS, PUTTING_PHASE_ORDER

    phase_scores = {
        p: PhaseScore(score=82, label=PUTTING_PHASE_LABELS[p], is_weakest=(p == "impact"))
...
async def _run_putting_analyze(req: AnalyzeRequest) -> AnalyzeResult:
    """推杆 mode 分发：mock 返回确定性报告；真实模式走 ``run_putting_analysis``。"""
    started_at = time.perf_counter()
    if settings.AI_ENGINE_MOCK_MODE:
        result = _mock_putting_result(req)
    else:
        try:
            from app.pipeline.putting.pipeline import run_putting_analysis

            result = await run_putting_analysis(req)
        except PipelineError as exc:
            log.warning(
                "putting_pipeline_error",
                analysis_id=req.analysis_id,
                code=exc.code,
                error=str(exc),
            )
