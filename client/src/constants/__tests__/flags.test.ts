...
import {
  ...
  PHASE2_CHIPPING_MODE_ENABLED_FLAG,
  ...
  PHASE2_PUTTING_MODE_ENABLED_FLAG,
...
describe('feature flags', () => {
  ...
  test('PHASE2_* defaults are false in source (运营手动开启)', () => {
    ...
    expect(PHASE2_PUTTING_MODE_ENABLED_FLAG).toBe(false)
    expect(PHASE2_CHIPPING_MODE_ENABLED_FLAG).toBe(false)
    expect(PHASE2_YARDAGE_BOOK_ENABLED_FLAG).toBe(false)
    expect(PHASE2_TRAINING_CATEGORIES_ENABLED_FLAG).toBe(false)
  })
