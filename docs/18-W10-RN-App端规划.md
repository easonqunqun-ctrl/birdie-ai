...
> ⚠️ **已废弃（2026-07-20）**：Taro-RN App 端路线已放弃，App 改用**独立 Flutter 工程** `app/`。
> 现行落地计划见 [`22-App-Flutter独立重写落地计划.md`](./22-App-Flutter独立重写落地计划.md)。
> 本文档仅作历史归档，其中的 `client/` 双端、rn-shell、`make client-check-rn` 等已不再适用。
