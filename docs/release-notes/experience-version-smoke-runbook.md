...
## 当前线上基线（2026-07-21 · 1.2.43 待上传 · 待提审）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.43**（整改微信审核拒因：登录页强制登录）                                                          |
| **Git / CVM**    | API `https://api.birdieai.cn/v1` · 后端无业务变更                                              |
| **本版变更（1.2.43）** | 登录页增加显著「暂不登录，先逛逛」（可返回/进首页访客态）；401 清 token 后回首页访客态，不再强跳登录页连环弹窗 |
| **当前任务**         | ① 上传开发版 **1.2.43** ② mp **提交审核**（仅人工） ③ 真机确认登录页有「先逛逛」且可浏览首页                                                                 |
