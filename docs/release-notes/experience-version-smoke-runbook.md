...
## 当前线上基线（2026-07-21 · 1.2.45 已上传）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.45**（DevTools CLI 已上传；约 1.3 MB）                                                          |
| **Git / CVM**    | `main@39006e9` · API 无业务变更                                              |
| **本版变更（1.2.45）** | 底部四 Tab 改自定义 tabBar：文案约 28rpx、图标 56rpx；兼容教练身份文案切换 |
| **当前任务**         | ① mp 选体验版用 **1.2.45**（仅人工） ② 真机确认底栏字号/图标更大且切换正常                                                                 |

> **上一版基线（1.2.44）**：分析参数底栏重叠 / 机位识别久等 / 伪试挥短段。
> **CLI 上传坑（重要）**：改用 `cli upload --project client/dist`。

**下一步（仅人工）**：体验版/提审用 **1.2.45**；真机确认底栏四键更大。
