...
## 当前线上基线（2026-09-12 · 1.2.46 待上传）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.46**（`make client-build-weapp-prod` 后由人工上传）                                                          |
| **Git / CVM**    | `main@25a4af5` · 欢迎包 + 评分诚实话术                                              |
| **本版变更（1.2.46）** | 国内外各前 100 名 × 100 次终身分析，用尽回落每月 3 次；报告页腕部代理诚实说明 |
| **当前任务**         | ① 开发者工具上传 `client/dist` 并选为体验版 ② 真机看首页配额「试用剩余 n / 100」③ 确认第 101 名起为每月 3 次                                                                 |

> **上一版基线（1.2.45）**：底部四 Tab 自定义 tabBar 放大。
> **CLI 上传坑（重要）**：改用 `cli upload --project client/dist`。

**下一步（仅人工）**：体验版用 **1.2.46**；真机确认欢迎包次数与会员入口。
