...
## 当前线上基线（2026-07-19 · 1.2.39 已上传 · 待选为体验版）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.39**（CLI 已上传；Tab 页开放胶囊转发 / 分享朋友圈）                                                                                      |
| **Git / CVM**    | API `https://api.birdieai.cn/v1` · `/v1/health` ok（后端仍为 1.2.38 收口逻辑）                                                     |
| **本版变更（1.2.39）** | 首页 / AI 教练 / 训练 / 我的：`enableShareAppMessage` + `enableShareTimeline`，转发落地首页                                              |
| **当前任务**         | ① mp **选为体验版** ② 真机确认 **1.2.39** + 胶囊「转发给朋友」可用 + 公测 P0                                                                     |


**下一步（仅人工）**：微信公众平台 → 版本管理 → 开发版本 **1.2.39** → **选为体验版** → 真机 smoke。
