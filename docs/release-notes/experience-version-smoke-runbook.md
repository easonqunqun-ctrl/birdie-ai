...
## 当前体验版基线（2026-06-04 · AI 教练访客预览 · 提审合规）

| 项 | 值 |
|----|-----|
| **小程序版本** | **1.2.34**（构建后上传；提审 / 体验版） |
| **API** | `https://api.birdieai.cn/v1` |
| **CVM backend** | `83e53aa`（本版无后端变更） |
| **本版变更** | AI 教练 Tab 未登录可浏览：欢迎语 + 示例对话 + 快捷问题 + 输入框；仅发送时弹窗引导登录（微信审核合规） |
| **待验** | 未登录进 AI 教练 Tab 不跳登录页；可见示例对话；点「登录发送」才弹登录 |

**发版后必做**：mp 后台上传 **1.2.34** → **提交审核**（或选为体验版先自测）→ 未登录验证 AI Tab 可浏览。

> **在哪勾选？** 用 Cursor / VS Code 打开 **本文件**，在编辑器里点 `- [ ]` 左侧方框，或把 `[ ]` 改成 `[x]`。

### 1.2.34 真机 Smoke 快捷勾选（提审合规 · 约 5 分钟）
...
- [ ] **未登录**打开小程序 → 切 **AI 教练** Tab → **不自动**跳转登录页
- [ ] 可见：欢迎语、**示例对话（预览）**、快捷问题、输入框
- [ ] 点快捷问题 → 填入输入框（不强制登录）
- [ ] 点 **「登录发送」** → 弹窗「登录后开始对话」→ 可选继续浏览 / 去登录
- [ ] 顶栏 **登录** 链接 → 进入登录页（主动行为）
...
### 1.2.33 真机 Smoke 快捷勾选（历史 · 约 15 分钟 · P0+P1）

#### 发版前确认

- [ ] 公众平台 **版本管理 → 1.2.33 已选为体验版**
- [ ] 真机扫码为体验版（非开发版）
- [ ] 真机 **关闭**「不校验合法域名」
- [ ] 关于/版本号显示 **1.2.33**
