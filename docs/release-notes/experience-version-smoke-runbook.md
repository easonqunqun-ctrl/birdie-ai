...
## 当前线上基线（2026-07-18 · 1.2.36 包已构建 · 待 mp 上传）

| 项 | 值 |
|----|-----|
| **小程序版本** | **1.2.36**（`client/dist` 已 `make client-build-weapp-prod`；**待**开发者工具上传 + 选为体验版） |
| **Git / CVM** | `307fdc0` · API `https://api.birdieai.cn/v1` · backend 已发（含 `membership_view` / `upgrade_cta_click` 白名单）· `/v1/health` ok |
| **本版变更（1.2.36）** | Batch-J：漏斗埋点 · 公测价值卡 · 本周主攻 · 练完再拍 · 对比晒进步 |
| **当前任务** | ① 微信开发者工具上传 `client/dist` → mp **选为体验版** ② 真机勾 **Batch-J 增量 + 公测 P0 + §D** |

**下一步（仅人工）**：开发者工具打开仓库 `client/dist` → 上传 → 公众平台选为体验版 → 真机 smoke。
