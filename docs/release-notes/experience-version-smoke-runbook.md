...
## 当前线上基线（2026-07-19 · 1.2.37 已上传 · 待选为体验版）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.37**（CLI 已上传，约 1.3 MB；**待**公众平台选为体验版）                                                                                |
| **Git / CVM**    | `86c9710` · API `https://api.birdieai.cn/v1` · ai_engine 新镜像 + backend/worker 热更已生效 · `/v1/health` ok · `DEFER_SKELETON_VIDEO=true` |
| **本版变更（1.2.37）** | 拍摄→分析提速：Celery 去掉重复 `/precheck`；骨骼视频异步出报告；报告页短轮询补骨骼                                                                      |
| **当前任务**         | ① mp **选为体验版** ② 真机确认 **1.2.37** + 抽测「出分更快 / 骨骼稍后出现」 + 公测 P0                                                            |


**下一步（仅人工）**：微信公众平台 → 版本管理 → 开发版本 **1.2.37** → **选为体验版** → 真机 smoke。
