...
## 当前线上基线（2026-07-19 · 发版中 · 1.2.37）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.37**（分析提速：内联预检 + 骨骼异步；待 CLI 上传 / 选为体验版）                                                                              |
| **Git / CVM**    | 发版中 · API `https://api.birdieai.cn/v1` · 须 backend + ai_engine + celery 同发                                                        |
| **本版变更（1.2.37）** | 拍摄→分析提速：Celery 去掉重复 `/precheck`；骨骼视频异步出报告；报告页短轮询补骨骼                                                                      |
| **当前任务**         | ① CVM 发版 ② 上传体验版 **1.2.37** ③ 真机确认版本 + 抽测「出分更快 / 骨骼稍后出现」                                                                   |


**下一步**：发版完成后 → 公众平台选为体验版 → 关于页确认 **1.2.37** → 收口公测 P0 / §D。
