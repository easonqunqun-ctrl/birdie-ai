...
## 当前线上基线（2026-07-20 · 1.2.40 已构建 · 待上传/选为体验版）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.40**（`client/dist` 已从干净 HEAD `61eb694` 构建；仅含训练页布局修复）                                                                  |
| **Git / CVM**    | API `https://api.birdieai.cn/v1` · `/v1/health` ok（后端未变，仍为 1.2.38 收口逻辑）                                              |
| **本版变更（1.2.40）** | 训练页「会员+无训练计划」空态：进步曲线/月历恢复左对齐（原被空态居中样式污染而整体看着歪）；会员无进步数据且本月无打卡时隐藏空的进步曲线/月历，空态 CTA 唱主角 |
| **当前任务**         | ① 开发者工具打开 `client/dist` **上传** ② mp **选为体验版** ③ 真机确认「关于/设置」版本号 **1.2.40** + 训练页空态不再歪                                        |

> **上一版基线（1.2.39）**：Tab 页 `enableShareAppMessage` + `enableShareTimeline`，转发落地首页。

**下一步（仅人工）**：微信开发者工具打开 `client/dist` → **上传** 为开发版本 **1.2.40** → 微信公众平台 → 版本管理 → **选为体验版** → 真机 smoke。
