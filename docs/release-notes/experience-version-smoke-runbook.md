...
## 当前线上基线（2026-07-21 · 1.2.42 已上传 · 待选为体验版 / 提审）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.42**（DevTools CLI 已上传开发版本；`client/dist` · AppID `wx045feb5e244fffae` · 约 1.3 MB）                                                          |
| **Git / CVM**    | `main@37385d4` · API `https://api.birdieai.cn/v1` · `/v1/health` ok · release-cvm 完成                                              |
| **本版变更（1.2.42）** | ① 协议合规：登录「我已阅读并同意」+ 未勾选禁用；首启同意页默认未勾选 ② adapters（kv/share/safeArea）与浮层定位收敛 ③ 拍摄校验走 `validatePickedVideo`；训练空态修复见 1.2.41 |
| **当前任务**         | ① mp **选为体验版**或**提交审核**（仅人工） ② 真机确认版本号 **1.2.42** + 协议勾选 + 训练空态                                                                 |

> **上一版基线（1.2.41）**：训练页空态移除进步曲线/会员卡偏位问题。
> **CLI 上传坑（重要）**：`cli upload --project client` 多次 `open` 后会把窗口 miniprogramRoot 搞乱成 `client/` 根 → `41002 appid missing`。**改用 `cli upload --project client/dist`。**

**下一步（仅人工）**：微信公众平台 → 管理 → 版本管理 → 开发版本 **1.2.42** → **选为体验版** 或 **提交审核** → 真机确认协议勾选交互。
