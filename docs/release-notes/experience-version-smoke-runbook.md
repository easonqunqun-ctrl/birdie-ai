...
## 当前线上基线（2026-09-13 · 1.2.47 待上传）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.47**（`make client-build-weapp-prod` 后由人工上传）                                                          |
| **Git / CVM**    | 注册起 3 个月不限次，到期后每月 3 次                                              |
| **本版变更（1.2.47）** | 取消前 100 名欢迎包；全员按注册日 3 个月免费体验，到期后每月 3 次、再多用收费 |
| **当前任务**         | ① 开发者工具上传 `client/dist` 并选为体验版 ② 真机看首页「免费体验至 ×月×日」③ 老号若注册已满 3 个月应见每月 3 次                                                                 |

> **上一版基线（1.2.46）**：球手对比库详情可播职业示范挥杆。
> **CLI 上传坑（重要）**：改用 `cli upload --project client/dist`。

**下一步（仅人工）**：体验版用 **1.2.47**；真机确认体验期文案与到期后配额。
