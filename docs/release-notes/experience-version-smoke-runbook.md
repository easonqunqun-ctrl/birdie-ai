...
## 当前体验版基线（2026-06-01 · 诊断兜底 + 低分训练建议）

| 项 | 值 |
|----|-----|
| **小程序版本** | **1.2.33**（构建后上传；公众平台 **选为体验版**） |
| **API** | `https://api.birdieai.cn/v1` |
| **CVM backend** | `83e53aa`（诊断兜底 + 最弱阶段 drill + 多挥误切 `76b0a91`） |
| **本版变更** | 低分无 issue 时按最弱阶段给训练建议；OTT/诊断静默失败修复；报告低分文案；多挥误切合并（`76b0a91`） |
| **待验** | 差挥杆报告有训练建议或弱项提示；单段视频不进 select-swing；低分不再显示「没有明显问题」 |

**发版后必做**：mp 后台 → **1.2.33 → 选为体验版** → 真机拍一段差挥杆，确认有诊断或训练建议。
