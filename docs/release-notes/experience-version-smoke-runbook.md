...
## 当前线上基线（2026-07-21 · 1.2.43 已上传 · 待提审）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.43**（DevTools CLI 已上传；`client/dist` · 约 1.3 MB）                                                          |
| **Git / CVM**    | `main@3c1737c` · API `https://api.birdieai.cn/v1` · 后端无业务变更                                              |
| **本版变更（1.2.43）** | 登录页增加显著「暂不登录，先逛逛」（可返回/进首页访客态）；401 清 token 后回首页访客态，不再强跳登录页连环弹窗 |
| **当前任务**         | ① mp **提交审核**（仅人工） ② 真机确认登录页有「先逛逛」且可浏览首页                                                                 |

> **上一版基线（1.2.42）**：协议勾选合规；审核仍拒「登录阶段无取消」。
> **CLI 上传坑（重要）**：改用 `cli upload --project client/dist`。

**下一步（仅人工）**：微信公众平台 → 管理 → 版本管理 → 开发版本 **1.2.43** → **提交审核** → 真机确认「暂不登录，先逛逛」。
