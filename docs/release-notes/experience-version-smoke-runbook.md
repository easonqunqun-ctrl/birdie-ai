...
## 当前线上基线（2026-07-20 · 1.2.40 已上传 · 待选为体验版）


| 项                | 值                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **小程序版本**        | **1.2.40**（DevTools CLI 已上传开发版本；从干净 HEAD `61eb694` 构建，仅含训练页布局修复）                                                          |
...
| **当前任务**         | ① mp **选为体验版**（仅人工） ② 真机确认「关于/设置」版本号 **1.2.40** + 训练页空态不再歪                                                                 |

> **上一版基线（1.2.39）**：Tab 页 `enableShareAppMessage` + `enableShareTimeline`，转发落地首页。

**下一步（仅人工）**：微信公众平台 → 管理 → 版本管理 → 开发版本 **1.2.40** → **选为体验版** → 真机 smoke。
