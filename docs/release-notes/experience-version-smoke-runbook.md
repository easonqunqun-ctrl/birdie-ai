...
## 当前线上基线（2026-06-04 · 1.2.34 已发布）

| 项 | 值 |
|----|-----|
| **小程序版本** | **1.2.34**（**已发布线上**；微信审核已通过） |
| **Git / CVM** | `2929329`（客户端）· 后端能力自 `83e53aa` 起 |
| **API** | `https://api.birdieai.cn/v1` |
| **本版变更** | AI 教练 Tab 未登录可浏览（访客预览）；此前 1.2.33 诊断兜底 + 多挥误切 + params 非阻塞 |
| **当前任务** | **线上真机 smoke 关项**（非提审、非上传） |

**下一步**：用**正式版**真机勾下方 **「线上待验 P0」**；全绿后关 **P2-W21-B**，并继续 Phase2 §D。

> **在哪勾选？** 用 Cursor / VS Code 打开 **本文件**，点 `- [ ]` 左侧方框，或把 `[ ]` 改成 `[x]`。

### 线上待验 P0（1.2.34 正式版 · 优先勾 · 约 20 分钟）
