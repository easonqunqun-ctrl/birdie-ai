...
| **1.1.2** | 已交付 | real_pipeline Runbook + ECS v1 节奏；**无客户端改动，无需提审**；版本号同步 1.1.2 |
| **1.2.0** | 待提审 | O-08 客户端 5s 硬预检（阻断/警告分级）；**需上传体验版** |
| **1.2.1** | 待提审 | O-09 等待页失败引导 + O-10 报告脚注；**需上传体验版** |
| **1.2.2** | 引擎/CI | ENG-04 ECS CI 回归；**无需小程序提审** |
| **1.2.3** | 已交付 | O-07 骨骼 FPS 编码链 |
| **1.2.4** | 待提审 | 原片/骨骼切换 + 去角标；**需上传体验版** |
| **1.2.4 引擎** | 待 CVM | FPS 硬门禁 + 评分标定；**需 `make release-cvm`** |
