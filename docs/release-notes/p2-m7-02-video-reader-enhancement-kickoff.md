...
| 不做 | 原因 |
| --- | --- |
| 不动客户端上传链路 | P2-M7-01 / P2-M7-03 覆盖；本文件仅 server-side |
| 不动算法（PoseEstimator / PhaseSplitter） | 上游产物（normalized.mp4 + fps + duration_sec）格式不变 |
...
- **质量门 warning code 机制**：`quality_warnings_from_scan` 已返回 `["low_light", "camera_shake", ...]` 字符串列表 → V2 可直接复用为 `engine_warnings[]` 的子集
...
| `ai_engine/tests/fixtures/real/` | （视频不入库） | 新增 6 类 v2 fixtures（详 §6） |
...
| FR-5 竖屏短边按设备分层（720 / 1080） | ❌ 硬编码 `TARGET_SHORT_SIDE = 720` | 与 P2-M7-01 ECS v2 联动：旗舰机型走 1080，老机型走 720（W17 推荐机型矩阵就位后切换） |
...
| 错误码 50120 | ❌ 未定义 | 同时入 `ai_engine/app/errors.py` + `docs/02 §11.1`（W17 PR） |
...
> - 本节 fixtures（≥24 段）用途 = 解码**回归**（codec / HDR / 慢动作格式覆盖）
...
| 类别 | 数量 | 设备 | 命名规范 | 用途 |
| --- | --- | --- | --- | --- |
| HEVC（H.265） | ≥5 | iPhone 15 / 14 默认 | `real/hevc_iphone15_<club>_<idx>.mov` | FR-1 |
| HEVC 10-bit HDR | ≥5 | iPhone 15 Pro HDR / 华为 Mate 60 HDR | `real/hdr_<device>_<club>_<idx>.mov` | FR-1 + FR-3 |
| VP9 | ≥3 | YouTube 1080p 60fps `.webm`（教练公开内容） | `real/vp9_yt_<club>_<idx>.webm` | FR-1 |
| 240fps 慢动作 | ≥5 | iPhone 慢动作模式 / Samsung Super Slow-mo | `real/slowmo240_<device>_<club>_<idx>.mov` | FR-2 + FR-4 |
| 120fps 高帧率 | ≥3 | 安卓 120fps（OnePlus / Pixel） | `real/highfps120_<device>_<club>_<idx>.mp4` | FR-2 |
| 24fps 电影模式 | ≥3 | iPhone 电影模式（默认 24fps） | `real/cinematic24_<device>_<club>_<idx>.mov` | FR-2（低帧率 dup-frame 验证） |

**总计 ≥24 段；与 ECS v2 不可重用**。

### 6.2 采集来源（合规）

| 路径 | 适用 | 备注 |
| --- | --- | --- |
| 内部自录 | 全部类别 | 优先；员工挥杆 + 教练助理协作，签内部数据使用同意书（同 P2-M7-01 §8.1 模板） |
...
`fixtures/real/` 在 v1（GolfDB face_on / dtl 各 1 段）基础上扩充 6 类：
- hevc_*  HEVC / H.265 容器
...
| **W17** | 真机回归（≥30 段，含推荐机型矩阵）；新增 50120 错误码 PR 同步 `docs/02 §11.1` + `ai_engine/app/errors.py` | engine_warnings collector 回流 `docs/03 §8.1` + `docs/02 §11.1` PR；本文件改 `superseded` 标记 | ☑ AC-1/2/3 全部达成（§3.2 验收口径）；☑ 文档回流 PR 合入；☑ Phase 2.1 接力书签可挂 |
...
| R-03 | TARGET_FPS 30→60 打破 V1 行为，灰度需 `engine_version` 兼容 | 高 | V1 报告与 V2 报告同环境共存时分数对比失真 | 强依赖 P2-M7-14 灰度框架；V2 上线前确认 `engine_version` 字段已写入 swing_analyses |
...
| Phase | W14-W17 持续采集 → Phase 2.1 持续扩 | W14-W17 一次性采足 ≥24 段，后续按需扩 |
