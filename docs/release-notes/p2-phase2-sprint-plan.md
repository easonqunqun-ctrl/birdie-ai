...
| **W26** | **Drill 内容** | M10-04 | tips 全库 + seed **30 条** + 训练页 tips UI；**视频** Trigger | **✅ A/C Done · B Trigger** |
...
| 2026-05-29 | **Phase D · M8-03**（学员绑定）：`coach_student_relations` 扩展 + 双向 opt-in API + 40312/40313/40915；invite/accept/visibility UI |
| 2026-05-29 | **Phase D · M8-03 follow-ups ✅**（#140）：shared-profile 真值 + 60 天 pending 过期 + 教练学员列表 UI |
| 2026-05-29 | **Phase D · M8-04**（报告批注）：text + video_ref；M8-03 active 师生关系守门；教练/学员 UI |
| 2026-05-29 | **Phase D · M8-05**（作业派发）：`coach_assigned_tasks` + drill 派发 + 训练 Tab 分组 + 完成同步 |
| 2026-05-29 | **Phase D · M8-06**（学员看板）：聚合 API + Redis 30s 缓存 + `needs_response` 排序 + 教练列表/详情 UI |
| 2026-05-29 | **Phase D · M8-07**（教学报告）：`course_session_recaps` + LLM 汇总 + PDF 水印导出 + 教练 UI |
| 2026-05-29 | **Phase D · M8-08**（内容审核）：`moderation_queue` + 批注/作业 hook + Admin 人工复核 + fail-safe |
| 2026-05-29 | **Phase D · M8-09**（教练无配额）：`request_role` 短路 + Redis 日限风控；`X-Role` / JWT role；无新 C API |
