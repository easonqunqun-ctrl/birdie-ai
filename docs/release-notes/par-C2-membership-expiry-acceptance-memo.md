...
| **代码版本** | `main @ HEAD`（会员激活 / 惰性降级 / 退款降级链路自 W7 起已上线） |
| **验收范围** | `docs/01 §8.3 退款策略 + §8.4 订阅管理`：会员到期降级、退款触发降级、关闭自动续费、配额回落 |
| **验收日期** | 2026-05-21 |
| **起草** | 工程侧自动化纪要 |
| **复核（待签）** | 产品 ▢　工程 ▢ |
...
4. 到期前订阅消息推送（3 天 / 1 天 / 0 天三档），用户可在公众平台收到提醒

### 1.2 隐含验收项

- **H1**：到期降级是**惰性**的（不依赖定时任务），任何读用户的请求都会触发；
- **H2**：降级幂等（重复读不会重复回落配额或乱发提醒）；
- **H3**：到期前 3 天 / 1 天的订阅消息推送有 Celery beat 入口，无订阅模板 ID 配置时**优雅 noop**；
...
| `backend/tests/test_payment_tasks.py` | `membership_pre_expiry_notify` beat 任务调度 |
