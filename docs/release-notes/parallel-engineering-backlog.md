...
> **总表**：未完项优先级与源码锚点汇总见 **`docs/19-产品开发迭代计划-当前队列.md`**；**生产核销 / 发版前置** 的紧急可执行项见该文档 **§二 紧急队列**。**全量未闭环（含 `docs/01` 全部未勾/部分项、Q 矩阵、parallel 逐条）合并排期主表**见该文档 **§六**；**代码已交付但 `docs/01` 未勾**见 **§6.5b DOC-\***（**DOC-04** 对应下文 C1–C3 验收纪要）。
