...
| 仓库 `main` HEAD | `ddaee8c` | Batch-J 1.2.36 已 push；CVM backend/worker/beat 已重建 |
| 客户端 | **1.2.36** 已是体验版（产品确认 2026-07-19） | 系统侧预检 ✅；公测 P0 / §D 真机收口中 |
| 埋点白名单 | ✅ 容器内已含 `membership_view` / `upgrade_cta_click`；`POST /v1/events` 抽检 accepted/rejected 正常 | PP-05 · 2026-07-19 复核 |

## 须人工真机（阻塞 PP-12 关项）

在微信体验版（**关闭**「不校验合法域名」）按 smoke runbook 勾选：

1. **1.2.36 公测免费 P0**（首页 banner / 不限次 / 教练未登录合规）
